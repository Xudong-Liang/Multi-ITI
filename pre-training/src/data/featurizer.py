...
def smiles_to_graph_tune(smiles, max_length=5, n_virtual_nodes=8, add_self_loop=True):
    ...
    # our code
    #if mol is None:
        # Create a graph with a single virtual node and no edges
    #    g = dgl.graph(([0], [0]))
    #    g.ndata['begin_end'] = torch.zeros((1, 2, d_atom_feats))
    #    g.ndata['edge'] = torch.zeros((1, d_bond_feats))
    #    g.ndata['vavn'] = torch.ones((1,), dtype=torch.int64) * n_virtual_nodes
    #    g.edata['path'] = torch.zeros((1, max_length), dtype=torch.int64)
    #    g.edata['lgp'] = torch.zeros((1,), dtype=torch.bool)
    #    g.edata['mgp'] = torch.zeros((1,), dtype=torch.bool)
    #    g.edata['vp'] = torch.ones((1,), dtype=torch.bool)
    #    g.edata['sl'] = torch.zeros((1,), dtype=torch.bool)
    #    return g
    new_order = Chem.rdmolfiles.CanonicalRankAtoms(mol)
    mol = Chem.rdmolops.RenumberAtoms(mol, new_order)
    # Featurize Atoms
